 ...
 */
  class Do :SendResultInterface,NomalResultInterface {
     ...
     */
    private fun nomalDo(){
        var data:DoModel = threadQueue[index] as DoModel
        if (data.thread == onUI){
            uiThread.post{
                data.call.invoke {
                    index+=1
     ...
     */
    private fun sendDo(){
        var data:DoSendModel = threadQueue[index] as DoSendModel
        if (data.thread == onUI){
            uiThread.post{
                data.call.invoke {
                    value ->
    ...
    private fun resultDo(value :Any){
        var data:DoResultModel = threadQueue[index] as DoResultModel
        if (data.thread == onUI){
            uiThread.post{
